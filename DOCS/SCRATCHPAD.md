...
|------|-------|-----------------|--------------|
| 2026-04-01 | Claude Haiku 4.5 | Anthropic Claude | **ChangelogScreen polish (Tier 1+2+3):** Staggered entry animations (160ms per card, `rememberEntryAnimations` pattern); left accent stripe (3dp gradient fade per tag color); glass highlight (2dp horizontal gradient top); improved spacing (20dp padding, 8–10dp gaps); tag emoji badges (8 tags with emoji: 📚 DOCS, 🎯 HUB, 🎪 SKILLS, 🎨 UI, 🚀 LAUNCH, 🏗️ FOUNDATION); interactive border color animation on tap (350ms tween); upgraded typography (`titleSmall` for "WHAT'S NEW", `titleLarge` for version labels); respects `LocalReduceMotion` accessibility. Added `tagEmoji` field to `ChangelogEntry` data class. `:app:compileDebugKotlin` + `:app:assembleDebug` green. |
