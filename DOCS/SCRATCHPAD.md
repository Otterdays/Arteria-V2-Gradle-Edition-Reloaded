...
0. **Research handoff ready:** Use `DOCS/agent_prompt_report_for_research.md` as the starting brief for another AI agent researching clicker/idle improvements.
...
**2026-04-29:** **Equipment silhouette + twin rings / Head slot (Composer):** Expanded loadout beyond four flat pockets to a **seven-slot paper-doll ordering** (**Head**, weapon/armor/accessory/Rings I-II, **Tool**) with **`EquipmentSlots.paperDollOrder`**; registry + XP/combat engines now multiply across **every** worn equip id via **`EquippedGear.allEquippedIdsForGameplay()`** / **`combatItemIds()`**; **`GameMetaEntity`** persists **`equippedHead`/`equippedRing`/`equippedRing2`** with **`MIGRATION_5_6`** (**Room v6**) and **`normalizeEquippedGearFromColumns()`** keeps legacy installs coherent when Jewelry definitions slide between slots. Released **v1.10.0 (13)** What's New card + **`DOCS/SBOM.md`** supersession/amendment paragraphs for **v6**; build `:app:compileDebugKotlin`, `:core:test` green.

**2026-04-29:** **SBOM reconciliation (Codex):** Audited `DOCS/SBOM.md` against live Gradle + Room schema. Updated stale **AGP 9.1.0** references to **9.2.0**, documented **GameDatabase version 5** and full `MIGRATION_1_2`…`4_5` chain + **release minify** row; preserved historical 2026-03-31 dependency-sweep bullets with a 2026-04-29 supersession note. No Maven coordinate bumps (current pins still match `app/build.gradle.kts`). **`[SUPERSEDED for schema headline]:`** Room **`GameDatabase` version 6** + **`MIGRATION_5_6`** appended later the same day for expanded equipment persistence — rely on **`DOCS/SBOM.md` Android Targets** row + amendment stack for authoritative version.
