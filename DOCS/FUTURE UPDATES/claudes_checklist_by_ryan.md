...
**`[AMENDED 2026-06-18]:`** **Canonical path: `DOCS/FUTURE UPDATES/claudes_checklist_by_ryan.md`** (this file). Legacy redirect: `DOCS/claudes_checklist_by_ryan.md`. Hub: **`DOCS/FUTURE UPDATES/README.md`**. Live product counts: **`DOCS/SUMMARY.md`** → Live Product Snapshot.
...
- [x] **6a.** Group items by category (Ores, Bars, Logs, Fish, Herbs, Potions, Salvage) with collapsible section headers **`[PARTIAL 2026-06-18]:`** heuristic categories + collapse in `BankScreen` (Grouped mode)
- [ ] **6b.** Item cards show quantity, item name, and a subtle icon placeholder — styled to match `ArteriaPalette` card system
- [x] **6c.** "Craftable now" badge — highlight items that are inputs to an affordable recipe the player can start immediately **`[DONE 2026-06-18]:`** `ItemUsageIndex` + bank card badge
- [x] **6d.** Search / filter bar — filter by item name or category **`[DONE 2026-04-01]:`** `BankScreen` search + sort menu
- [ ] **6e.** Tap an item → mini detail sheet: what it's used for, which skill produces it, current quantity **`[PARTIAL 2026-06-18]:`** `BankItemDetailDialog` with produced-by / used-in
...
- [ ] **7e.** Skill lore blurb — short flavour text pulled from `SkillId.description`, shown below the hero header **`[DONE 2026-06-18]:`** `SkillDetailScreen`
...
- [x] **8e.** Loot drop feed — scrolling mini-cards of drops that go to bank automatically **`[PARTIAL 2026-06-18]:`** Recent drops strip on `CombatScreen`; full scroll feed still open
